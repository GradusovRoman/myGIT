...
public class Network implements Closeable {
            ...
            public void run() {
                while (!Thread.currentThread().isInterrupted()) {
                    try {
                        ...
                        String login = MessagePatterns.parseConnectedMessage(text);
                        if (login != null) {
                            messageReciever.userConnected(login);
                            continue;
                        }

                        login = MessagePatterns.parseDisconnectedMessage(text);
                        if (login != null) {
                            messageReciever.userDisconnected(login);
                            continue;
                        }

                        Set<String> users = MessagePatterns.parseUserList(text);
                        if (users != null) {
    ...
    public void requestConnectedUserList() {
        sendMessage(MessagePatterns.USER_LIST_TAG);
    }
