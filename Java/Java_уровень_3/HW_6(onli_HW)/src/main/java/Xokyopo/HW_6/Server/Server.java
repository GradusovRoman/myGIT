...
import Xokyopo.HW_6.MessageParser.MessageParser;
...
public class Server extends Application implements InputMessage {
    ...
    private Logger logger = Logger.getRootLogger(); // т.к всеравно завязано на фреймворке log4j
    ...
    public void stop() throws Exception {
        ...
        this.logger.warn("Сервер остановлен");
    }

    public void connection() {
        try {
            ServerSocket socket = new ServerSocket(this.port);
            this.logger.info("Сервер запущен ждет подключений");
            while (true) {
                new LoginServer(socket.accept(), this);
            }
        } catch (IOException e) {
            this.logger.warn("Ошибка запуска сервера", e);
        }
    }
    ...
    public void subscribe(ServerInternetConnection _client) {
        ...
        if (_client != null) {
            ...
            this.logger.info("Клиент с ником " + _client.getClientName() + "подключился");
        } else {
            this.logger.fatal("Вызов метода subscribe(ServerInternetConnection _client) с передачей ему null значения");
        }
    }
    ...
    public void inputMessage(String _msg) {
        ...
        if (message.get(Tag.MESSAGETAG) != null) {
            this.logger.info("Пользователь \"" + message.get(Tag.NICKNAMETAG) + "\" прислал сообщение");
        } else {
            this.logger.info("Пользователь \"" + message.get(Tag.NICKNAMETAG) + "\" прислал команду" + _msg);
        }
    ...
    public void printServerInfo(String _msg) {
        ...
        DataBase.insert(date.getTime(), _msg);
    }
